...
def get_pitch_yaw_roll(car, forward, up=UP):
    ...
    if dot(car.up, up) < -.8 and dot(car.forward,
                                     forward) > .8:  # abs(roll_vel) < .3 and dot(car.up, up) < -.98 and dot(car.forward, forward) > .8:
        if roll == 0:
            roll = 1
        roll *= 1e10
    ...
    def state_kickoff(self, s):
        ...
        self.kickoff_config = {
            KickOffPosition.FAR_BACK: [
                                          StrategyComponent(duration=1.0,
                                                            callback=lambda: self.kickoff_boost_to_ball(s)),
                                          StrategyComponent(duration=0.1, callback=lambda: self.kickoff_drive_right()),
                                          StrategyComponent(duration=1.1,
                                                            callback=lambda: self.kickoff_boost_to_ball(s)),
                                      ] + hit_with_corner_strat,
            ...
            KickOffPosition.CORNER: [
                                        StrategyComponent(duration=1.70,
                                                          callback=lambda: self.kickoff_boost_to_ball(s)),
                                    ] + hit_with_corner_strat,
        }
    ...
    def get_target_pos(self, s):
        ...
        predicted_slice = bisect_ball_prediction(
            ...
            lambda slice: not could_reach_target_in_time(
                ...
                slice.game_seconds - s.time + 300.0 / dist(s.car.pos,
                                                           Vec3(slice.physics.location.x, slice.physics.location.y,
                                                                slice.physics.location.z))
            )
        )
    ...
    def state_flip_towards_ball(self, s: EasyGameState):
        ...
        if s.car.on_ground:
            ...
            if s.car.double_jumped:
                ...
                if s.car.boost > 50 and dot(s.car.forward, desired_forward) > 0.95 and dist(target_pos,
                                                                                            s.car.pos) > 500:
                    out.boost = True
            else:
                WAIT_ALTITUDE = 0.1
